...
#define BASH_GETRANDOM getrandom
#else
...
static ssize_t
bash_getrandom (void *buf, size_t len, unsigned int flags)
{
  int oflags;
  ssize_t r;
...
#define BASH_GETRANDOM bash_getrandom
#endif
...
get_urandom32 (void)
{
  ...
  if (BASH_GETRANDOM ((void *)&ret, sizeof (ret), GRND_NONBLOCK) == sizeof (ret))
